...
function buildMd(data) {
  ...
  const yaml = [
    ...
    `notebookId: "${escapeYaml(data.notebookId)}"`,
    `notebookPages: "${escapeYaml(data.notebookPages)}"`,
  ...
  const body = [
    ...
    `- 第一著者: ${safeDash(data.firstAuthor)}`,
    `- 最終著者: ${safeDash(data.lastAuthor)}`,
    `- 責任者（対応担当）: ${safeDash(data.owner)}`,
    `- ジャーナル: ${safeDash(data.journal)}`,
    ...
    '',
    '## 実験ノート',
    `- ノート番号: ${safeDash(data.notebookId)}`,
    `- 該当ページ: ${safeDash(data.notebookPages)}`,
    '',
    '## 倫理・保管',
...
async function loadMdFile() {
  ...
  const data = {
    ...
    notebookPages: fm.notebookPages ?? '',
    irb: fm.irb ?? '',
